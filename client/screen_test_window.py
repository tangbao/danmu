...
class ScreenTestWindow(QtGui.QWidget):
    def __init__(self, parent, screen_id):
        ...
        l_font = QtGui.QFont()
        l_font.setPointSize(48)
        l_font.setFamily('Microsoft Yahei')
        l.setFont(l_font)

        l_palette = QtGui.QPalette()
        l_palette.setColor(QtGui.QPalette.WindowText, Qt.Qt.black)
        l.setPalette(l_palette)

        l.setText('{0}'.format(screen_id))
